...
**基础路线已完成（18 课 / 300 测试全绿）**，现进入"未来方向 A-G"阶段（A Spring Boot / B 补薄弱 / C 读源码 / D JVM / E 并发深入 / F 工程化 / G 综合实战），教学约定不变。
...
## 助学约定
- **会话入口协议**：用户说"继续学习"时，先按顺序读取以下对齐进度（**禁止主动创建/修改任何文件**）：
  1. `docs/00-learning-roadmap.md` -- 18 课已完成，看总览 / 知识地图 / 当前学习方向（A-G）；读码生词/报错可查 `docs/java-reading-guide.md`
  2. `/memories/repo/java-pra-traps.md` -- 看末尾"学习进度"行 + 最近踩的坑
  3. 最近的 `docs/dayN-learning-notes.md` -- 看上一阶段末尾"下一步"
  读完用一句话确认进度；若用户已选未来方向 A-G，直接开讲该方向第一个概念点。
- **未来方向选择**：基础路线完成后，用户从 A-G 方向自选（见 `docs/00-learning-roadmap.md` 第九节）：
  A=Spring Boot 过渡 / B=补薄弱点（泛型通配符、并行流陷阱、虚拟线程 pin、设计模式）/ C=读开源源码 / D=JVM 深入 / E=并发深入 / F=工程化 / G=综合实战。
  开新方向时仍遵循"概念先行 + 单次一个概念点 + 你实现我校验"，涉及已学知识点可直接引用对应 Lab（如 IoC 理解 -> 第十八课）。
...
- **求助分级（重要）**：根据用户措辞响应不同
  ...
  - 用户说 **"校验"** -> **只跑** `gradle test` + 反馈通过/失败原因；若用户没补测试（测试未覆盖新方法=假通过），**直接代写覆盖测试**后再跑（见下方"测试归属"），代写后简述覆盖了哪些边界
...
- **测试归属**：简单输入输出测试 agent 可代写；**有难度的测试**（边界、异常、并发、PECS、Pattern）由用户自写，agent 只跑 `gradle test` 校验。**校验时发现测试未覆盖新方法（假通过）→ agent 直接代写覆盖测试**；**用户显式要求代写时（"你写测试/测试你写"），agent 代写**（即使是并发类），并在代写后简述测试覆盖了哪些边界。
